...
import { sendSms, smsOPT, smsVerify } from '../config/sendSMS';
import {
  IDecodedToken,
  IUser,
  IGgPayload,
...
const authCtrl = {
  register: async (req: Request, res: Response) => {
    try {
      ...
      if (validateEmail(account)) {
        ...
        return res.json({
          msg: 'Success! Please check your number.',
        });
      }
    } catch (err: any) {
  ...
  login: async (req: Request, res: Response) => {
    try {
      ...
      const user = await Users.findOne({ account });
      if (!user)
        return res.status(400).json({ err: 'This account does not exist.' });
  ...
  loginSMS: async (req: Request, res: Response) => {
    try {
      ...
      const data = await smsOPT(phone, 'sms');
